...
export async function GET() {
  ...
  const [missions, attempts, sessions, mocks, errors, revisions, achievements, syllabus, plannerTasks, plannerNotes] =
    await Promise.all([
      ...
      db.plannerTask.findMany({ where: { profileId: profile.id } }),
      db.plannerNote.findMany({ where: { profileId: profile.id } }),
    ]);
  ...
  const backup = {
    ...
    plannerNotes: plannerNotes.map((n) => ({
