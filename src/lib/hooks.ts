...
export function usePlannerDay(date: Date) {
  ...
  return useQuery({
    ...
    queryFn: () =>
      jfetch<{ tasks: PlannerTask[]; note: string }>(`/api/planner?date=${key}`),
  });
}

export function useCreatePlannerTask(date: Date) {
  const qc = useQueryClient();
  const key = toDateKey(date);
  return useMutation({
    mutationFn: (body: {
      ...
      jfetch<{ task: PlannerTask }>("/api/planner", {
        method: "POST",
        body: JSON.stringify({ date: key, ...body }),
      }),
    onSuccess: () => qc.invalidateQueries({ queryKey: ["planner", key] }),
  });
}

export function useTogglePlannerTask(date: Date) {
  const qc = useQueryClient();
  const key = toDateKey(date);
  return useMutation({
    mutationFn: (id: string) =>
      jfetch(`/api/planner/${id}`, { method: "PATCH", body: JSON.stringify({ toggle: true }) }),
    onMutate: async (id: string) => {
      await qc.cancelQueries({ queryKey: ["planner", key] });
      const previous = qc.getQueryData<{ tasks: PlannerTask[]; note: string }>(["planner", key]);
      qc.setQueryData<typeof previous>(["planner", key], (old) => {
        if (!old) return old;
        return {
          ...old,
    ...
    onError: (_err, _id, context) => {
      if (context?.previous) qc.setQueryData(["planner", key], context.previous);
    },
    onSettled: () => qc.invalidateQueries({ queryKey: ["planner", key] }),
  });
}

export function useUpdatePlannerTask(date: Date) {
  const qc = useQueryClient();
  const key = toDateKey(date);
  return useMutation({
    mutationFn: ({ id, ...body }: { id: string } & Partial<PlannerTask>) =>
      jfetch(`/api/planner/${id}`, { method: "PATCH", body: JSON.stringify(body) }),
    onSuccess: () => qc.invalidateQueries({ queryKey: ["planner", key] }),
  });
}

export function useDeletePlannerTask(date: Date) {
  const qc = useQueryClient();
  const key = toDateKey(date);
  return useMutation({
    mutationFn: (id: string) => jfetch(`/api/planner/${id}`, { method: "DELETE" }),
    onMutate: async (id: string) => {
      await qc.cancelQueries({ queryKey: ["planner", key] });
      const previous = qc.getQueryData<{ tasks: PlannerTask[]; note: string }>(["planner", key]);
      qc.setQueryData<typeof previous>(["planner", key], (old) =>
...
export function useSavePlannerNote(date: Date) {
  const qc = useQueryClient();
  const key = toDateKey(date);
  return useMutation({
    mutationFn: (content: string) =>
      jfetch(`/api/planner/note`, { method: "PUT", body: JSON.stringify({ date: key, content }) }),
    onSuccess: () => qc.invalidateQueries({ queryKey: ["planner", key] }),
  });
}
...
export function useRegister() {
  ...
  return useMutation({
    ...
    onSuccess: async () => {
      // Refetch auth FIRST, while its cache entry still exists and still has
      // an active observer (page.tsx's useAuth()). This is what flips
      // `account`/`hasProfile` and lets page.tsx's effect move us off the
      // Auth screen immediately.
      //
      // We used to call qc.clear() before this, but clear() wipes the
      // "auth" query out of the cache too — so the refetchQueries call right
      // after had nothing left to match and silently did nothing. The login
      // API call still succeeded (cookie set correctly), which is why a
      // manual refresh "fixed" it: a fresh page load re-runs the auth query
      // from scratch. But without a refresh, the UI just sat on the login
      // screen forever.
      await qc.refetchQueries({ queryKey: ["auth"] });
      // Now wipe every other cached query so no previous user's data leaks
      // into the new session (leave "auth" alone — it's already correct).
...
export function useLogin() {
  ...
  return useMutation({
    ...
    onSuccess: async () => {
      // See useRegister for why we refetch "auth" before clearing anything else.
      await qc.refetchQueries({ queryKey: ["auth"] });
      qc.removeQueries({ predicate: (query) => query.queryKey[0] !== "auth" });
    },
  });
...
export function useLogout() {
  ...
  return useMutation({
    ...
    onSuccess: async () => {
      // Same fix as login/register: refetch "auth" first (now returning
      // account: null) so the UI reacts immediately, then clear everything
      // else so the next person to sign in on this device never sees stale
